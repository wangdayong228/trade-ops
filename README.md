...
本地操作界面对交易所列表加载、预检、已有策略加载、状态刷新和确认请求统一显示详细错误，包括 HTTP 状态、稳定错误码、脱敏后的原始错误类型/错误码/消息，以及用于关联 stdout 日志的请求 ID。旧格式 JSON 错误、非 JSON 响应、网络错误和响应校验错误也会明确区分。

界面详情与 stdout 使用相同的安全边界：当前加载的 API key、secret 和 password/passphrase 会替换为 `[Redacted]`，并且不会显示堆栈、任意错误属性、完整环境变量或原始 CCXT 请求/响应。确认接口返回 `202` 只表示后台任务已受理；之后发生的异步执行错误应结合界面中的持久化策略状态、失败码和 stdout 日志诊断。
