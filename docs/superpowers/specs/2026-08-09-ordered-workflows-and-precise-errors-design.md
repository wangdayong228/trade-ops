...
- 执行：`ORDER_TOPOLOGY_MISMATCH`、`ORDER_INTENT_MISMATCH`、`ORDER_NOT_SUBMITTED`、`ORDER_SUBMISSION_UNKNOWN`、`ORDER_SNAPSHOT_MISMATCH`、`ORDER_STATUS_MISMATCH`、`ORDER_NOT_FOUND`、`ORDER_NO_FILL`、`ORDER_AVERAGE_PRICE_UNAVAILABLE`、`HEDGE_ORDER_REJECTED`、`HEDGE_ORDER_CANCELED`、`INVARIANT_VIOLATION`。
...
- `execution-precheck-service`：每个新订单的账户、市场、名义金额、余额和网关准备检查。
